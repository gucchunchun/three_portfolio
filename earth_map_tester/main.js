...
const SPHERE_COLOR = 0x256E93; //blue
const LATITUDE_COUNT = SPHERE_RADIUS * Math.PI * 2; //0.5=>DOT_SIZE=0.25
const DOT_SIZE = 0.25;
const DOT_DENSITY = 0.9;
const DOT_SEGMENT = 16;
const DOT_COLOR = 0x1B3B3D;
const INTERSECTED_DOT = 0x3C8185;
const PIN_COLOR = 0xEA9D3D;
const INTERSECTED_PIN = 0xFFD748;
...
class Globe {
    ...
    add_pin(lat, long) {
        ...
        target.material.color = new THREE.Color(0xEA9D3D);
...
let canvas_height = w_width*WINDOW_RATIO;
let renderer, camera, controls, scene, raycaster, globe, intersects;
let INTERSECTED;
let theta = 0;
function init() {
    //renderer
...
function render() {
    ...
    if ( intersects.length > 0 ) {
        if (intersects[ 0 ].object.name == "dot") {
            if (INTERSECTED) {
                INTERSECTED.material.color = new THREE.Color(INTERSECTED.currentHex);
            }
            INTERSECTED = intersects[ 0 ].object;
            INTERSECTED.currentHex = INTERSECTED.material.color;
            intersects[ 0 ].object.material.color =  new THREE.Color(INTERSECTED_DOT);
        }
    } else {
        if ( INTERSECTED ) {
            INTERSECTED.material.color = new THREE.Color(INTERSECTED.currentHex);
